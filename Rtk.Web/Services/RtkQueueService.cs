...
public sealed record RtkTaskState(
    string TaskId,
    RtkTaskStatus Status,
...
{
    // 使用有界队列，避免突发请求导致内存无限增长
    private readonly Channel<RtkTask> _channel = Channel.CreateBounded<RtkTask>(new BoundedChannelOptions(100)
    {
        SingleReader = true,
        SingleWriter = false,
    ...
    private readonly ConcurrentDictionary<string, RtkTaskState> _taskStates = new();
    ...
    public async ValueTask EnqueueAsync(RtkTask task)
    {
        ...
        _taskStates[task.TaskId] = new RtkTaskState(task.TaskId, RtkTaskStatus.Queued, now);
        await _channel.Writer.WriteAsync(task);
        _logger.LogInformation($"[队列] 任务 {task.TaskId} 已加入等待队列");
    }

    public RtkTaskState? GetTaskState(string taskId)
    {
        _taskStates.TryGetValue(taskId, out var state);
        return state;
    }
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        await foreach (var task in _channel.Reader.ReadAllAsync(stoppingToken))
        {
            ...
            _taskStates[task.TaskId] = new RtkTaskState(
                task.TaskId,
                RtkTaskStatus.Running,
                _taskStates.TryGetValue(task.TaskId, out var current) ? current.CreatedAt : startedAt,
                StartedAt: startedAt);
            ...
            try
            {
                ...
                _taskStates[task.TaskId] = new RtkTaskState(
                    task.TaskId,
                    RtkTaskStatus.Completed,
                    _taskStates.TryGetValue(task.TaskId, out current) ? current.CreatedAt : startedAt,
                    StartedAt: startedAt,
                    FinishedAt: DateTimeOffset.UtcNow);
            }
            catch (TimeoutException ex)
            {
                _logger.LogError(ex, $"[超时] 任务 {task.TaskId} 超时");
                _taskStates[task.TaskId] = new RtkTaskState(
                    task.TaskId,
                    RtkTaskStatus.TimedOut,
                    _taskStates.TryGetValue(task.TaskId, out current) ? current.CreatedAt : startedAt,
                    StartedAt: startedAt,
                    FinishedAt: DateTimeOffset.UtcNow,
                    Error: ex.Message);
