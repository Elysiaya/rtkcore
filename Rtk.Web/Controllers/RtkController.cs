...
public class RtkController : ControllerBase
{
    ...
    {
        var state = _queueService.GetTaskState(taskId);
        if (state is not null)
        {
            return Ok(new
            {
                ...
                state.FinishedAt,
                state.Error,
                ResultUrl = state.Status == RtkTaskStatus.Completed ? $"/api/rtk/download/{taskId}" : null
            });
        }
        ...
        {
            return Ok(new { Status = RtkTaskStatus.Completed.ToString(), ResultUrl = $"/api/rtk/download/{taskId}" });
        }

        return NotFound(new { Status = "NotFound", Message = "任务不存在或已过期" });
    }
